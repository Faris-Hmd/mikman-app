...
import { Ticket, Server, Layers, Users, Laptop, Settings, ChevronLeft, ChevronRight, LogOut, LayoutDashboard, TrendingUp, User, Home } from 'lucide-react';
  ...
  return (
    <>
      <SidebarContainer className={isMobile ? '' : 'desktop-nav'}
        style={{ width: isMobile ? '100%' : (collapsed ? '68px' : '230px'), transition: isMobile ? 'none' : 'width 0.25s cubic-bezier(0.4, 0, 0.2, 1)', height: '100%', backgroundColor: isMobile ? 'transparent' : 'var(--card-bg)', borderRight: isMobile ? 'none' : '1px solid var(--glass-border)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', flexShrink: 0, overflowX: 'hidden', boxSizing: 'border-box', padding: isMobile ? '0 16px 12px' : '0 10px 10px', zIndex: 90 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', width: '100%', flexShrink: 0 }}>
          <Link to="/" onClick={() => { if (isMobile && onClose) onClose(); }} title={t('header.routerSelection')}
            style={{ display: 'flex', alignItems: 'center', justifyContent: collapsed ? 'center' : 'flex-start', boxSizing: 'border-box', height: isMobile ? '52px' : '60px', marginTop: '0', marginLeft: isMobile ? '-16px' : '-10px', marginRight: isMobile ? '-16px' : '-10px', paddingLeft: isMobile ? '16px' : '12px', paddingRight: isMobile ? '16px' : '12px', borderBottom: '1px solid var(--glass-border)', marginBottom: '6px', width: isMobile ? 'calc(100% + 32px)' : 'calc(100% + 20px)', cursor: 'pointer', transition: 'background-color 0.2s', textDecoration: 'none' }}>
          ...
          {!isMobile && (
            <button onClick={toggleCollapse} title={collapsed ? t('sidebar.expandMenu') : t('sidebar.collapseMenu')}
              style={{ display: 'flex', alignItems: 'center', justifyContent: collapsed ? 'center' : 'flex-start', gap: collapsed ? '0' : '10px', padding: '6px 10px', borderRadius: '8px', backgroundColor: 'transparent', color: 'var(--text-muted)', border: 'none', transition: 'all 0.2s ease', fontSize: '12.5px', fontWeight: '500', cursor: 'pointer', width: '100%', boxSizing: 'border-box', marginBottom: '4px' }}>
        ...
        <div className="custom-scroll" style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '3px', width: '100%', boxSizing: 'border-box', paddingRight: collapsed ? '0' : '2px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', width: '100%', marginTop: 'auto', paddingTop: '6px', flexShrink: 0 }}>
          {collapsed ? (
            <>
              {user && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', alignItems: 'center', width: '100%' }}>
                  <Link to="/account" onClick={() => { if (isMobile && onClose) onClose(); }} title={t('sidebar.accountDetails')}
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2px 0', borderRadius: '6px', cursor: 'pointer', boxSizing: 'border-box', transition: 'background-color 0.2s', width: '100%', textDecoration: 'none' }}>
                    <div style={{ width: '26px', height: '26px', borderRadius: '50%', overflow: 'hidden', border: '1.5px solid var(--primary)', backgroundColor: 'var(--card-bg)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <button onClick={handleSignOutRequest} title={t('sidebar.signOutShort') || 'Sign Out'}
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'transparent', border: 'none', borderRadius: '6px', padding: '6px', color: '#ef4444', cursor: 'pointer', width: '100%', transition: 'background-color 0.2s' }}>
                    <LogOut size={16} />
                  </button>
                </div>
              )}
            ...
            <>
              {user && (
                <div style={{ paddingTop: '6px', borderTop: '1px solid var(--glass-border)', display: 'flex', flexDirection: 'column', gap: '6px', boxSizing: 'border-box', width: '100%' }}>
                  <Link to="/account" onClick={() => { if (isMobile && onClose) onClose(); }} title={t('sidebar.accountDetails')}
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-start', gap: '8px', padding: '4px 6px', borderRadius: '8px', cursor: 'pointer', boxSizing: 'border-box', transition: 'background-color 0.2s', textDecoration: 'none' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                      <span style={{ fontSize: '12px', fontWeight: '600', color: 'var(--foreground)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{userDisplayName}</span>
                  ...
                  <button onClick={handleSignOutRequest}
                    style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', backgroundColor: 'rgba(239, 68, 68, 0.08)', border: '1px solid rgba(239, 68, 68, 0.2)', borderRadius: '8px', padding: '5px 8px', color: '#ef4444', fontSize: '11px', fontWeight: '600', cursor: 'pointer', transition: 'all 0.2s ease', boxSizing: 'border-box', whiteSpace: 'nowrap' }}>
                    <LogOut size={14} />
                    <span>{t('sidebar.signOutShort') || 'Sign Out'}</span>
                  </button>
                </div>
              )}
              <div style={{ textAlign: 'center', fontSize: '9.5px', color: 'var(--text-muted)', marginTop: '2px', paddingTop: '4px', borderTop: '1px solid var(--glass-border)', lineHeight: '1.2' }}>
