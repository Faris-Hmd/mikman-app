...
import { Ticket, Server, Layers, Users, Laptop, Settings, ChevronLeft, ChevronRight, Sun, Moon, LogOut, LayoutDashboard, TrendingUp, ArrowRightLeft, User, Globe, Home } from 'lucide-react';
...
interface SidebarProps {
  ...
  isConnected: boolean;
  onConnectRouter: (router: RouterConfig) => void;
  theme: 'dark' | 'light';
  toggleTheme: () => void;
...
export default function Sidebar({
  activeRouter, savedRouters, isConnected, onConnectRouter, theme, toggleTheme,
  isRouterConnected, onDisconnect, user, isMobile = false, onClose,
  onSignOutRequest, onProfileClick
}: SidebarProps) {
  ...
  const globalItems = [
    { label: t('sidebar.mainPage'), icon: Home, href: '/', onClick: () => { if (isRouterConnected) onDisconnect(); }, isActive: pathname === '/' && !isRouterConnected },
    { label: t('sidebar.accountDetails'), icon: User, href: '/account', isActive: pathname === '/account' }
  ];

  const routerItems = activeRouter ? [
    { href: `/${activeRouter.id}`, label: t('sidebar.dashboard'), icon: LayoutDashboard },
    { href: `/${activeRouter.id}/revenue`, label: t('sidebar.revenue'), icon: TrendingUp },
    { href: `/${activeRouter.id}/vouchers`, label: t('sidebar.vouchers'), icon: Ticket },
    { href: `/${activeRouter.id}/profiles`, label: t('sidebar.profiles'), icon: Server },
    { href: `/${activeRouter.id}/batch`, label: t('sidebar.batchPrint'), icon: Layers },
    { href: `/${activeRouter.id}/users`, label: t('sidebar.users'), icon: Users },
    { href: `/${activeRouter.id}/aps`, label: t('sidebar.devices'), icon: Laptop },
    { href: `/${activeRouter.id}/records`, label: t('sidebar.audit'), icon: Server }, /* was ClipboardList */
    { href: `/${activeRouter.id}/settings`, label: t('sidebar.settings'), icon: Settings },
  ] : [];
  ...
  return (
    <SidebarContainer className={isMobile ? '' : 'desktop-nav'}
        ...
        {globalItems.map((item, idx) => {
          ...
          return (
            <Link key={idx} to={item.href} title={collapsed ? item.label : undefined}
              onClick={() => { if (item.onClick) (item.onClick as any)(); onClose?.(); }}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', width: '100%', marginTop: 'auto', paddingTop: '10px', flexShrink: 0 }}>
        {isRouterConnected && (
          <button onClick={() => { onDisconnect(); if (onClose) onClose(); }} title={collapsed ? t('sidebar.switchRouter') : undefined}
            style={{ display: 'flex', alignItems: 'center', justifyContent: collapsed ? 'center' : 'flex-start', gap: collapsed ? '0' : '10px', padding: collapsed ? '7px 10px' : '6px 12px', borderRadius: '10px', color: 'var(--text-muted)', backgroundColor: collapsed ? 'transparent' : 'rgba(var(--primary-rgb), 0.05)', border: collapsed ? 'none' : '1px solid var(--glass-border)', transition: 'all 0.2s ease', fontWeight: '500', fontSize: '12.5px', width: '100%', cursor: 'pointer', boxSizing: 'border-box', textAlign: 'left' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, width: collapsed ? '20px' : 'auto' }}><ArrowRightLeft size={16} color="var(--primary)" /></div>
            {!collapsed && <span>{t('sidebar.switchRouter')}</span>}
          </button>
        )}
          ...
          <>
            ...
            {user && (
              <div style={{ marginTop: '4px', paddingTop: '4px', borderTop: '1px solid var(--glass-border)', display: 'flex', flexDirection: 'column', gap: '8px', alignItems: 'center', width: '100%' }}>
                <div onClick={() => { if (onProfileClick) onProfileClick(); if (isMobile && onClose) onClose(); }} title={t('sidebar.accountDetails')}
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '4px 0', borderRadius: '8px', cursor: 'pointer', boxSizing: 'border-box', transition: 'background-color 0.2s', width: '100%' }}>
                ...
                <button onClick={() => { if (onSignOutRequest) onSignOutRequest(); }}
          ...
          <>
            {user && (
              <div style={{ marginTop: '2px', paddingTop: '6px', borderTop: '1px solid var(--glass-border)', display: 'flex', flexDirection: 'column', gap: '8px', boxSizing: 'border-box', width: '100%' }}>
                <div onClick={() => { if (onProfileClick) onProfileClick(); if (isMobile && onClose) onClose(); }} title={t('sidebar.accountDetails')}
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-start', gap: '10px', padding: '4px 8px', borderRadius: '8px', cursor: 'pointer', boxSizing: 'border-box', transition: 'background-color 0.2s' }}>
