...
import { AuthContext, AccountInfo } from './AuthContext';
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Fallback user data when logged in but server response has no specific userData structure
  const defaultUserData: UserData | null = user ? {
    email: user.email || '',
    approved: true,
    expiresAt: null,
    maxRouters: 10,
  ...
  const activeUserData = data?.userData || cachedUserData || (isSubLoading ? null : defaultUserData);
